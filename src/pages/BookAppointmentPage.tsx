    ...
    return <>
      ...
      <section className="pt-32 pb-24 bg-muted/20">
        <div className="container mx-auto px-4 max-w-xl">
          <div className="rounded-3xl border border-primary/20 bg-card p-8 md:p-10 text-center shadow-sm">
            <div className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-primary/10 text-primary"><ShieldCheck className="h-7 w-7" /></div>
            <span className="mt-6 inline-block text-sm font-semibold text-primary uppercase tracking-widest">Secure booking</span>
            <h1 className="mt-3 text-3xl md:text-4xl font-display font-extrabold">Sign in before booking a consultation.</h1>
            <p className="mt-4 text-muted-foreground leading-relaxed">Appointments are linked to a verified Google account so we can reduce spam and send confirmations to the correct email address.</p>
            <Button onClick={handleGoogleSignIn} disabled={signingIn} size="lg" className="mt-7 w-full h-12 bg-gradient-primary text-primary-foreground">
              {signingIn ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <LogIn className="mr-2 h-4 w-4" />}
              {signingIn ? "Signing in..." : "Continue with Google"}
            </Button>
            <p className="mt-4 text-xs text-muted-foreground">No separate password is required for appointment booking.</p>
          </div>
        </div>
      </section>
    </>;
  ...
  return <>
    ...
    <section className="pt-32 pb-14 bg-muted/20">
      <div className="container mx-auto px-4 lg:px-8 max-w-5xl">
        ...
        <div className="mt-5 inline-flex items-center gap-2 rounded-full border border-border bg-card px-3 py-1.5 text-xs text-muted-foreground"><ShieldCheck className="h-3.5 w-3.5 text-primary" />Signed in as <span className="font-semibold text-foreground">{user.email}</span></div>
      </div>
    </section>
      ...
      <div className="container mx-auto px-4 lg:px-8 max-w-5xl grid lg:grid-cols-[1fr_320px] gap-8 items-start">
        <form onSubmit={handleSubmit} className="bg-card rounded-2xl p-6 md:p-8 border border-border shadow-sm space-y-6">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <div><label className="text-sm font-medium mb-2 block">Email *</label><Input required type="email" value={form.email} readOnly className="bg-muted/40" maxLength={120} /></div>
          </div>
          <div className="grid sm:grid-cols-2 gap-4">
